  ...
  <div>
    ...
    >
      ...
      <div class="flex gap-8 items-center overflow-x-no-scrollbar">
        ...
        />
        <icon-button
          ref="visitChartDatePickerIconRef"
          @click="showDatePicker = true"
          icon="date"
          class="grow-0 shrink-0"
        />
        <n-date-picker
          ref="visitChartDatePickerRef"
...
defineOptions({ name: 'VisitChart' })

const showDatePicker = ref<boolean>(false)
const visitChartDatePickerRef = ref(null)
const visitChartDatePickerIconRef = ref(null)

const { el } = useClick()

watch(el, (newValue) => {
  if (!showDatePicker.value) return
  const inputWrapperEl: any = (visitChartDatePickerIconRef.value as any).$el
  const datePickerEl: any = (visitChartDatePickerRef.value as any).$el
...
async function getChartData() {
  startLoading()
  const { data } = await fetchReportVisit(fromDate.value, toDate.value)
  if (data) {
    setTimeout(() => {
      setChartData(data)
      endLoading()
